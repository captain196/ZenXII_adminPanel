...
| # | Task | Accept |
|---|---|---|
| P9.1 | ⛔ **BLOCKED — no PDF→PNG step exists.** Per-script rendering was verified at **G0.2/G0.3** (Lohit registers, 8 distinct subsets embed, conjuncts and matra reordering correct in all 8 scripts) — but on fixtures, and never compared to reference PNGs. | ⛔ A reference-PNG suite needs a rasteriser; `SCHOOL_DOCUMENT_ECOSYSTEM_RESEARCH` records there is **no PDF→PNG on the Ohio box**. The accept's real point — *"regenerating a reference is a reviewed change, never a convenience"* — is already honoured by the **golden-file** discipline (P2.8), which carries that warning in its own harness. |
...
| P9.3 | ⚠️ **CODES DONE, LOAD-TRIPPING NOT.** Typed codes exist and are pinned: `E_PAGE_OVERFLOW`, `E_IMAGE_SOURCE`, `E_CONFLICT`, all matching `^E_[A-Z_]+$` so a caller never has to substring-match an English sentence. Caps are declared and asserted sane, with their G0.6 derivation. | ⚠️ `MAX_PAGES` is enforced and throws; **`MAX_MEMORY` and `MAX_SECONDS` are `ini_set`/`set_time_limit` ceilings, not throwing paths** — tripping them needs a load harness, not a unit test. **Not counted.** |
