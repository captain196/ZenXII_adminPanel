...
| P6.4 | ✅ **DONE 2026-09-02 — and it was BROKEN, not merely unproven.** The adapter read `runTransaction()` off `raw_client()`, which returns a **`FirestoreRestClient` that has no such method** — every real activate would have fatalled on the first click. And the closure ignored the `Transaction` object it was handed, writing through the plain non-transactional helpers, so nothing would have been in the transaction anyway. Neither showed up in tests, because the double supplied exactly the capability production lacked. | ✅ Rebuilt on `:commit`, the all-or-nothing primitive the REST client **does** have. activate() commits the **complete assignment** — winner set, every other template nulled — as one batch, so two concurrent activates cannot interleave into a half-state. 4 tests: one commit not two; the batch names the winner and nulls every incumbent; both orderings leave exactly one active; a rejected commit leaves the incumbent alone. ⚠️ This proves **completeness**; Firestore's atomicity under real contention still wants an emulator drill with a control that can fail. |
...
| P9.2 | ✅ **DONE 2026-09-02.** **Save** was already proven — two concurrent saves produce exactly one conflict and the first writer's edit survives (`DocTemplateServiceTest`), and the client now surfaces that 409 as a refusal that **keeps the draft dirty** rather than reporting a save that did not happen (E2E **U7**). | ✅ **Activate** is now proven to the same standard, once the two defects above were fixed: the invariant *exactly one active template per document type* holds after **either** ordering of two activates. |
