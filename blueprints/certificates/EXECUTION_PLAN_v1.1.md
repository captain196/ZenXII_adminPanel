...
| P1.6 | `Doc_templates` controller skeleton + routes + **CSRF wiring** + `_remap` capability map | G0.7 | `controllers/Doc_templates.php`, `config/routes.php` | Every route enforces its capability grade; POST without token → 403 |
| P1.7 | `Doc_audit` — append-only, **writes into `auditLogs`** with `module: 'doc_templates'` (A1) | P1.1 | `libraries/Doc_audit.php` | Events appear in the **existing** Audit Logs viewer; no update/delete endpoint exists |
