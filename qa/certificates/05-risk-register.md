...
| **R7** | **A bad publish cannot be undone.** `activate()` always uses `publishedVersion`; there is no path back to an earlier version. | **P1** | ⚑ CONTESTED — may be intended | T0-12 |
| **R8** | **Archiving the active template silently leaves a type unissuable.** No sibling check, no warning. | **P1** | Open | T0-11 |
