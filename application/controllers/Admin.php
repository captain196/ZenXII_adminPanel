...
class Admin extends MY_Controller
{
    ...
    public function switch_session(): void
    {
        ...
        // (View-vs-Global decoupling is a separate parked module per
        // [[session_propagation_crosssystem]]; Step 2 hardens the existing
        // global-write behavior, doesn't decouple it. Step 6 retires this
        // method entirely.)
        ...
        // ── SC-Step2 (Session Convergence — 2026-06-02): lock + CAS ──
        // Same `sessions` lock and __updateTime precondition pattern as
        // School_config::set_active_session (the canonical writer per Q1
        // Option A). Serializes concurrent switch_session calls across
        // admins AND serializes against School_config::add/delete/archive/
        // set_active_session via the shared filesystem lock path.
        // PHP userdata is now mutated ONLY after the Firestore commit
        // succeeds, eliminating PHP/Firestore divergence on commit failure.
        $lock = $this->_config_lock_acquire('sessions');
        if ($lock === null) {
            log_message('error',
                "ACC_CONCURRENT_REJECTED endpoint=admin/switch_session "
                . "schoolId={$this->school_id} actor={$this->admin_id} "
                . "reason=lock_timeout session={$new_year}");
            $this->json_error(
        ...
        try {
            ...
            // Bypasses the userdata `available_sessions` cache which can be
            // stale relative to Firestore canonical.
            $fsSchool = $this->firebase->firestoreGet('schools', $this->school_id);
            if (!is_array($fsSchool)) {
                $this->json_error('School profile is not yet initialised.', 503);
                return;
            }
            ...
            // Whitelist — must be in this school's canonical sessions[]
            if (!in_array($new_year, $sessions, true)) {
                $this->json_error('Session not available for your school.', 403);
                return;
            }

            // Refuse switching INTO an archived session
            if (in_array($new_year, $archived, true)) {
                log_message('error',
                    "ACC_SESSION_SWITCH_REJECTED schoolId={$this->school_id} "
                    . "target={$new_year} reason=archived actor={$this->admin_id}");
                $this->json_error("Session {$new_year} is archived. Unarchive it before activation.", 409);
                return;
            }

            // Idempotent — already active, no write, no log_audit noise
            if ($priorActive === $new_year) {
                // Refresh PHP userdata anyway (caller may have stale cache)
                $this->session->set_userdata([
                    'session'         => $new_year,
                    'current_session' => $new_year,
                    'session_year'    => $new_year,
                ]);
                $this->json_success([
                    'session_year' => $new_year,
                    'idempotent'   => true,
                    'message'      => "Active session is already {$new_year}.",
                ]);
                return;
            }

            // CAS commit — Firestore __updateTime precondition prevents
            // cross-process races (e.g., concurrent set_active_session,
            // add_session, archive_session). DOC-MERGE-FIX: emit merge=true
            // + only the fields we want to change, so other doc fields
            // (sessions[], classes, streams, profile) remain intact.
            $ops = [[
                'op'         => 'update',
                'collection' => 'schools',
                'docId'      => $this->school_id,
                'merge'      => true,
                'data'       => [
            ...
            try {
                ...
                log_message('error',
                    "ACC_SESSION_SWITCH_COMMIT_FAILED endpoint=admin/switch_session "
                    . "schoolId={$this->school_id} session={$new_year} "
            ...
            if (!$committed) {
                log_message('error',
                    "ACC_CONCURRENT_REJECTED endpoint=admin/switch_session "
                    . "schoolId={$this->school_id} actor={$this->admin_id} "
                    . "reason=cas_failed prior={$priorActive} attempted={$new_year}");
                $this->json_error(
            ...
            // PHP userdata updated ONLY after successful Firestore commit.
            // Pre-Step-2 order wrote PHP userdata BEFORE Firestore — a
            // commit failure left PHP/Firestore divergent.
            $this->session->set_userdata([
    ...
    /**
     * SC-Step2 (2026-06-02): filesystem-lock helper duplicated from
     * School_config::_config_lock_acquire (L320-343). Same lock-file path
     * convention so cross-controller calls hitting the same path serialize
     * at the OS level. When Step 6 retires switch_session entirely, this
