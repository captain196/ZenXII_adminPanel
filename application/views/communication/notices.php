    ...
    CM.ajax('communication/get_notices', {}, function(r) {
        ...
        if (!list.length) {
            ...
            list.forEach(function(n) {
                ...
                html += '<tr>' +
                    '<td><strong>' + CM.esc(n.title) + '</strong><div style="font-size:11px;color:var(--t3);margin-top:2px">' + CM.esc((n.description||'').substring(0,80)) + '</div></td>' +
                    ...
                    '<td><button class="cm-btn cm-btn-sm cm-btn-primary" onclick="NTC.edit(\'' + CM.esc(n.id) + '\')"><i class="fa fa-pencil"></i></button> ' +
                    '<button class="cm-btn cm-btn-sm cm-btn-danger" onclick="NTC.del(\'' + CM.esc(n.id) + '\')"><i class="fa fa-trash"></i></button></td>' +
                    '</tr>';
            });
        }
