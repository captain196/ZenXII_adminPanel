...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .badge{display:inline-block;font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);border:1px solid rgba(56,189,248,.35);background:rgba(56,189,248,.08);padding:3px 9px;border-radius:999px;margin-bottom:14px}
...
<body>
  <div class="card">
    ...
    <?php if (!empty($must_change)): ?>
      <div class="notice">
        Your password was reset (or this is a new account). You're required to set
        a new one before accessing the Super Admin panel.
      </div>
    <?php endif; ?>
    ...
    <form id="cpwForm" autocomplete="off">
      ...
      <div class="who">Signed in as <?= htmlspecialchars((string) ($sa_name ?: '')) ?> (<?= htmlspecialchars((string) $sa_id) ?>)</div>
    </form>
  </div>
