...
.badge{display:inline-flex;align-items:center;gap:6px;font-size:10.5px;letter-spacing:.08em;text-transform:uppercase;
  font-weight:700;color:var(--brand);border:1px solid var(--brand-ring);background:var(--brand-dim);
...
<body>
  ...
  <div class="card">
    <div class="brandrow">
      <div class="brandmark"><i class="fas fa-user-shield"></i></div>
      <div>
    ...
    <span class="badge"><i class="fas fa-crown"></i>Super Admin Panel</span>
    ...
    <?php if (!empty($must_change)): ?>
      <div class="notice">
        ...
        <span>Your password was reset (or this is a new account). You're required to set a new one before accessing the Super Admin panel.</span>
      </div>
    <?php endif; ?>
    ...
    <form id="cpwForm" autocomplete="off">
      ...
      <div class="who"><i class="fas fa-user"></i>Signed in as <?= htmlspecialchars((string) ($sa_name ?: '')) ?> (<?= htmlspecialchars((string) $sa_id) ?>)</div>
    </form>
  </div>
...
(function(){
  ...
  const CSRF    = <?= json_encode((string) ($sa_csrf_token ?? '')) ?>;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const res = await fetch('<?= site_url('superadmin/change_my_password') ?>', {
        method: 'POST',
        headers: {
      ...
      if (res.ok && (json.status === 'success' || json.success === true)) {
        ...
        const target    = json.redirect || '<?= base_url('superadmin/dashboard') ?>';
