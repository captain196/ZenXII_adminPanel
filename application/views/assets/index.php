...
$can_edit   = function_exists('has_permission') ? has_permission('Assets','edit')   : true;
$can_manage = function_exists('has_permission') ? has_permission('Assets','manage') : true;
?>
...
<section class="content"><div class="ast-wrap">
  ...
  <?php if(!$can_edit): ?>
  <div class="rx-ro-banner"><span class="rx-ro-ic">🔒</span> View-only access — you can browse Asset records but cannot add, edit, or delete.</div>
  ...
  <div class="ast-panel<?= $at === 'registry' ? ' active' : '' ?>" id="panelAssets">
    <div class="ast-card"><div class="ast-card-title"><span>Asset Registry</span><button class="ast-btn ast-btn-primary" onclick="AST.openModal()" <?= $can_edit?'':'disabled' ?>><i class="fa fa-plus"></i> Register Asset</button></div>
  ...
  <div class="ast-panel<?= $at === 'categories' ? ' active' : '' ?>" id="panelCats">
    <div class="ast-card"><div class="ast-card-title"><span>Asset Categories</span><button class="ast-btn ast-btn-primary" onclick="AST.openCatModal()" <?= $can_edit?'':'disabled' ?>><i class="fa fa-plus"></i> Add</button></div>
  ...
  <div class="ast-panel<?= $at === 'assignments' ? ' active' : '' ?>" id="panelAssign">
    <div class="ast-card"><div class="ast-card-title"><span>Asset Assignments</span><button class="ast-btn ast-btn-primary" onclick="AST.openAsnModal()" <?= $can_edit?'':'disabled' ?>><i class="fa fa-plus"></i> Assign</button></div>
  ...
  <div class="ast-panel<?= $at === 'maintenance' ? ' active' : '' ?>" id="panelMaint">
    <div class="ast-card"><div class="ast-card-title"><span>Maintenance Records</span><button class="ast-btn ast-btn-primary" onclick="AST.openMntModal()" <?= $can_edit?'':'disabled' ?>><i class="fa fa-plus"></i> Add Record</button></div>
  ...
  <div class="ast-panel<?= $at === 'depreciation' ? ' active' : '' ?>" id="panelDep">
    <div class="ast-card"><div class="ast-card-title"><span>Depreciation Report</span><button class="ast-btn ast-btn-amber" onclick="AST.computeDep()" <?= $can_edit?'':'disabled' ?>><i class="fa fa-calculator"></i> Run Monthly Depreciation</button></div>
...
<div class="ast-modal-bg" id="astModal"><div class="ast-modal">
  ...
  <button class="ast-btn ast-btn-primary" onclick="AST.save()" style="width:100%;margin-top:8px" <?= $can_edit?'':'disabled' ?>>Save Asset</button>
</div></div>
...
<div class="ast-modal-bg" id="acatModal"><div class="ast-modal" style="width:420px">
  ...
  <button class="ast-btn ast-btn-primary" onclick="AST.saveCat()" style="width:100%;margin-top:8px" <?= $can_edit?'':'disabled' ?>>Save</button>
</div></div>
...
<div class="ast-modal-bg" id="astAsnModal"><div class="ast-modal" style="width:420px">
  ...
  <button class="ast-btn ast-btn-primary" onclick="AST.saveAsn()" style="width:100%;margin-top:8px" <?= $can_edit?'':'disabled' ?>>Assign</button>
</div></div>
...
<div class="ast-modal-bg" id="mntModal"><div class="ast-modal">
  ...
  <button class="ast-btn ast-btn-primary" onclick="AST.saveMnt()" style="width:100%;margin-top:8px" <?= $can_edit?'':'disabled' ?>>Save</button>
</div></div>

<script>
var _AST_CFG={BASE:'<?= base_url() ?>',CSRF_NAME:'<?= $this->security->get_csrf_token_name() ?>',CSRF_HASH:'<?= $this->security->get_csrf_hash() ?>',activeTab:'<?= $at ?>',CAN_EDIT:<?= $can_edit?'true':'false' ?>,CAN_MANAGE:<?= $can_manage?'true':'false' ?>};
document.addEventListener('DOMContentLoaded', function(){
(function(){
  'use strict';
  var BASE=_AST_CFG.BASE,CN=_AST_CFG.CSRF_NAME,CH=_AST_CFG.CSRF_HASH;
  var categories=[],assets=[];
  var CAN_EDIT=_AST_CFG.CAN_EDIT, CAN_MANAGE=_AST_CFG.CAN_MANAGE;
  ...
  function loadCats(){skel('acatTbody',5);getJSON('assets/get_categories').done(function(r){if(r.status!=='success'){$('#acatTbody').html('<tr><td colspan="5" style="text-align:center;color:var(--rose)">'+escH(r.message||'Failed to load')+'</td></tr>');return}categories=r.categories||[];
    var html='';categories.forEach(function(c){html+='<tr><td>'+escH(c.id)+'</td><td>'+escH(c.name)+'</td><td>'+c.depreciation_rate+'%</td><td>'+escH(c.method)+'</td><td><button class="ast-btn ast-btn-sm ast-btn-primary" onclick="AST.editCat(\''+c.id+'\')"'+EDIT_ATTR+'><i class="fa fa-pencil"></i></button> <button class="ast-btn ast-btn-sm ast-btn-danger" onclick="AST.delCat(\''+c.id+'\')"'+DEL_ATTR+'><i class="fa fa-trash"></i></button></td></tr>'});
  ...
  function loadAssets(){skel('astTbody',9);getJSON('assets/get_assets').done(function(r){if(r.status!=='success'){$('#astTbody').html('<tr><td colspan="9" style="text-align:center;color:var(--rose)">'+escH(r.message||'Failed to load')+'</td></tr>');return}assets=r.assets||[];
    var html='';assets.forEach(function(a){
      ...
      html+='<td><button class="ast-btn ast-btn-sm ast-btn-primary" onclick="AST.edit(\''+a.id+'\')"'+EDIT_ATTR+'><i class="fa fa-pencil"></i></button> <button class="ast-btn ast-btn-sm ast-btn-danger" onclick="AST.del(\''+a.id+'\')"'+DEL_ATTR+'><i class="fa fa-trash"></i></button></td></tr>';
    });
  ...
  function loadAssignments(){skel('asnTbody',7);getJSON('assets/get_assignments').done(function(r){if(r.status!=='success'){$('#asnTbody').html('<tr><td colspan="7" style="text-align:center;color:var(--rose)">'+escH(r.message||'Failed to load')+'</td></tr>');return}
    var html='';(r.assignments||[]).forEach(function(a){
      ...
      var act=a.status==='Active'?'<button class="ast-btn ast-btn-sm ast-btn-danger" onclick="AST.returnAsn(\''+a.id+'\')"'+EDIT_ATTR+'><i class="fa fa-undo"></i></button>':'—';
  ...
  function loadMaint(){skel('mntTbody',8);getJSON('assets/get_maintenance').done(function(r){if(r.status!=='success'){$('#mntTbody').html('<tr><td colspan="8" style="text-align:center;color:var(--rose)">'+escH(r.message||'Failed to load')+'</td></tr>');return}
  ...
  function loadDep(){skel('depTbody',7);getJSON('assets/get_depreciation_report').done(function(r){if(r.status!=='success'){$('#depTbody').html('<tr><td colspan="7" style="text-align:center;color:var(--rose)">'+escH(r.message||'Failed to load')+'</td></tr>');return}
  ...
  AST.editCat=function(id){if(!CAN_EDIT)return;var c=categories.find(function(x){return x.id===id});if(!c)return;$('#acId').val(c.id);$('#acName').val(c.name);$('#acRate').val(c.depreciation_rate);$('#acMethod').val(c.method);$('#acatModalTitle').text('Edit Category');$('#acatModal').addClass('show')};
  AST.saveCat=function(){if(!CAN_EDIT){toast('You do not have edit permission',false);return}var b=btnOn('#acatModal .ast-btn-primary');post('assets/save_category',{id:$('#acId').val(),name:$('#acName').val(),depreciation_rate:$('#acRate').val(),method:$('#acMethod').val()}).done(function(r){r=typeof r==='string'?JSON.parse(r):r;if(r.status==='success'){toast(r.message,true);AST.closeCatModal();loadCats()}else toast(r.message,false)}).always(function(){btnOff(b)})};
  AST.delCat=function(id){if(!CAN_MANAGE){toast('You do not have delete permission',false);return}if(!confirm('Delete?'))return;post('assets/delete_category',{id:id}).done(function(r){r=typeof r==='string'?JSON.parse(r):r;if(r.status==='success'){toast(r.message,true);loadCats()}else toast(r.message,false)})};
  ...
  AST.saveAsn=function(){if(!CAN_EDIT){toast('You do not have edit permission',false);return}var b=btnOn('#astAsnModal .ast-btn-primary');post('assets/save_assignment',{asset_id:$('#asnAssetId').val(),assigned_to:$('#asnTo').val(),assign_type:$('#asnType').val()}).done(function(r){r=typeof r==='string'?JSON.parse(r):r;if(r.status==='success'){toast(r.message,true);AST.closeAsnModal();loadAssignments()}else toast(r.message,false)}).always(function(){btnOff(b)})};
  AST.returnAsn=function(id){if(!CAN_EDIT){toast('You do not have edit permission',false);return}if(!confirm('Return this asset?'))return;post('assets/return_assignment',{assignment_id:id}).done(function(r){r=typeof r==='string'?JSON.parse(r):r;if(r.status==='success'){toast(r.message,true);loadAssignments()}else toast(r.message,false)})};
  ...
  AST.saveMnt=function(){if(!CAN_EDIT){toast('You do not have edit permission',false);return}var b=btnOn('#mntModal .ast-btn-primary');post('assets/save_maintenance',{id:$('#mntId').val(),asset_id:$('#mntAssetId').val(),type:$('#mntType').val(),description:$('#mntDesc').val(),cost:$('#mntCost').val(),date:$('#mntDate').val(),next_due:$('#mntNext').val(),vendor:$('#mntVendor').val(),status:$('#mntStatus').val()}).done(function(r){r=typeof r==='string'?JSON.parse(r):r;if(r.status==='success'){toast(r.message,true);AST.closeMntModal();loadMaint()}else toast(r.message,false)}).always(function(){btnOff(b)})};

  // Depreciation
  AST.computeDep=function(){if(!CAN_EDIT){toast('You do not have edit permission',false);return}if(!confirm('Run monthly depreciation for all active assets? This creates an accounting journal entry.'))return;var b=btnOn('#panelDep .ast-btn-amber');post('assets/compute_depreciation',{}).done(function(r){r=typeof r==='string'?JSON.parse(r):r;if(r.status==='success'){toast(r.message,true);loadDep();loadAssets()}else toast(r.message,false)}).always(function(){btnOff(b)})};
