...
<title>Student Password Reset — GraderIQ</title>
...
<div class="fp-card">
    <div class="fp-logo">
        <h1><i class="fas fa-user-graduate" style="margin-right:8px;"></i>GraderIQ</h1>
    ...
    <div class="fp-step active" id="step1">
        <div class="fp-field">
            <label class="fp-label"><i class="fas fa-envelope" style="margin-right:4px;"></i>Parent / Guardian Email</label>
            <input type="email" class="fp-input" id="fpEmail" placeholder="e.g. parent@email.com" autofocus>
        </div>
        ...
        <div class="fp-info">Enter the email address registered with your child's account.<br>An OTP will be sent for verification.</div>
    </div>

    <!-- STEP 2: Select Account + Enter OTP -->
    <div class="fp-step" id="step2">
        <div class="fp-info" style="margin-bottom:14px;">
            OTP sent to <strong id="fpEmailMasked"></strong>
        </div>

        <!-- Account list (shown when multiple accounts) -->
        <div id="acctSection" style="display:none;">
            <label class="fp-label"><i class="fas fa-users" style="margin-right:4px;"></i>Select Student Account</label>
    ...
    <div class="fp-step" id="step3">
        <div class="fp-info" style="margin-bottom:14px;">
            Resetting password for <strong id="fpSelectedName"></strong> (<strong id="fpSelectedId"></strong>)
        </div>
        <div class="fp-field">
            <label class="fp-label"><i class="fas fa-lock" style="margin-right:4px;"></i>New Password</label>
            <div class="fp-pw-wrap">
                <input type="password" class="fp-input" id="fpNewPw" placeholder="Min 6 characters" minlength="6">
        ...
        <div class="fp-field">
            ...
            <div class="fp-pw-wrap">
                <input type="password" class="fp-input" id="fpConfirmPw" placeholder="Re-enter password" minlength="6">
    ...
    <div class="fp-step" id="step4">
        <div class="fp-success-icon">
            ...
            <p style="font-size:12px;color:var(--text2);margin-top:8px;">
                Student ID: <strong id="fpDoneId" style="color:var(--brand3);"></strong><br>
...
function sendOtp(){
    ...
    postJson('admin_login/student_send_otp',{email:email},function(r){
...
function renderAccounts(){
    var section = document.getElementById('acctSection');
    var list = document.getElementById('acctList');
    list.innerHTML = '';

    if(accounts.length <= 1){
        // Single account — auto-select, hide selector
    ...
    accounts.forEach(function(a, i){
        ...
        card.innerHTML =
            '<input type="radio" name="acctSelect" value="'+a.userId+'" id="acct'+i+'">' +
            '<div class="acct-info">' +
                '<div class="acct-name">'+a.name+'</div>' +
                '<div class="acct-detail">' +
                    'Class '+a.className+' '+a.section +
                    (a.schoolDisplayName ? ' — '+a.schoolDisplayName : '') +
                '</div>' +
            '</div>' +
            '<span class="acct-id">'+a.userId+'</span>';

        card.addEventListener('click', function(){
            document.querySelectorAll('.acct-card').forEach(function(c){c.classList.remove('selected');});
...
function verifyOtp(){
    ...
    if(!selectedUserId){showAlert('Please select a student account.');return;}

    btnLoad('btnVerifyOtp',true);
    postJson('admin_login/student_verify_otp',{email:email,otp:otp,user_id:selectedUserId},function(r){
        btnLoad('btnVerifyOtp',false);
        if(r.status==='success'){
            resetToken=r.resetToken;
            document.getElementById('fpSelectedName').textContent=selectedName;
            document.getElementById('fpSelectedId').textContent=selectedUserId;
            showStep(3);
        } else {
...
function resetPassword(){
    ...
    postJson('admin_login/student_reset_password',{user_id:selectedUserId,reset_token:resetToken,new_password:pw},function(r){
        btnLoad('btnResetPw',false);
        if(r.status==='success'){
            document.getElementById('fpDoneId').textContent=selectedUserId;
            showStep(4);
        } else {
