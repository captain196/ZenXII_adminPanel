...
<div class="imp-wrap">
    ...
    <div class="imp-card" id="mapCard">
        ...
        <div class="alert" style="background:#eef2ff;color:#3730a3;border:1px solid #c7d2fe;border-radius:8px;padding:9px 13px;font-size:.84rem;margin-bottom:12px;">
            <i class="fa fa-magic"></i> Auto-filled <b><?= (int) $learnedApplied ?></b> column<?= $learnedApplied == 1 ? '' : 's' ?> from this school's previous imports. Your choices here are remembered for next time.
        </div>
        <?php endif; ?>
    ...
    <div class="imp-card imp-hidden" id="prevCard">
        ...
        <div style="background:var(--bg3,#f9fafb);border:1px solid var(--border,#e5e7eb);border-radius:8px;padding:12px 14px;margin:12px 0;">
            ...
            <label style="font-size:.86rem;cursor:pointer;">
                <input type="radio" name="dupPolicy" value="update"> Update their info <span style="color:var(--t3,#9ca3af);">(refresh contact/parent details; class &amp; fees untouched)</span>
            </label>
            <div style="margin-top:10px;">
                <button class="imp-btn ghost" id="dupCheckBtn" style="padding:6px 13px;font-size:.83rem;"><i class="fa fa-search"></i> Check for existing students</button>
                <span id="dupMeta" style="font-size:.82rem;color:var(--t3,#6b7280);margin-left:8px;"></span>
            </div>
        </div>
...
(function () {
    ...
    var DUPES = {};         // rowIndex -> {duplicate, existingId, reason}
    var dirty = false;
    ...
    // ── Canonical rows from current mapping ──────────────────────────────
    ...
    // ── Which canonical fields to show in the preview ────────────────────
        ...
        for (var i = 0; i < n; i++) {
            ...
            var dupBadge = (dupInfo && dupInfo.duplicate)
                ? ' <span title="' + esc(dupInfo.reason || 'duplicate') + (dupInfo.existingId ? ' — ' + esc(dupInfo.existingId) : '')
                    + '" style="background:#e0e7ff;color:#3730a3;border-radius:10px;padding:1px 7px;font-size:.72rem;font-weight:600;"><i class="fa fa-clone"></i> dup</span>'
                : '';
    ...
    el('dupCheckBtn').addEventListener('click', function () {
        ...
        var btn = this; setBtnLoading(btn, true, 'Checking…');
    ...
    // Keep the "will be skipped/updated" wording in sync with the policy.
    ...
    el('commitBtn').addEventListener('click', function () {
        ...
        // Send every row (server skips error rows authoritatively) plus the
        // header→field mapping so this school's layout is remembered.
        var map = currentMapping();
        ...
        // Chunked commit — each batch is its own request, so a large roster
        // never hits a single long request that a proxy/PHP would kill, and we
        // can show real progress. Cross-batch duplicates are still caught
        // because earlier batches write the phone index before later ones run.
    ...
    // Download a CSV of just the error rows (client-side) so the school can fix & re-upload.
