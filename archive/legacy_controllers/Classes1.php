...
<!-- <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script> -->
<script>
    var additionalSubjectStudents = <?= json_encode($additionalSubjectStudents); ?>;

document.addEventListener("DOMContentLoaded", function () {
    const subjectItems = document.querySelectorAll(".optional-subject-item");

    subjectItems.forEach(subject => {
        subject.addEventListener("click", function () {
            const selectedSubject = this.textContent.trim(); // Get subject name

            // Convert allowed user IDs to strings to ensure correct comparison
            const allowedUserIds = (additionalSubjectStudents[selectedSubject] || []).map(String);

            // Select table rows dynamically
            ...
            tableRows.forEach(row => {
                let userId = row.getAttribute("data-user-id")?.trim(); // Ensure userId is clean
                let isMatch = allowedUserIds.includes(userId);


                if (isMatch) {
                    row.style.display = ""; // Show row if user ID matches
                }
            });
        });
    });

    // Show all students when "Show All" is clicked
    document.getElementById("showAll").addEventListener("click", function () {
        document.querySelectorAll("tbody tr[data-user-id]").forEach(row => row.style.display = "");
    });
});


document.getElementById('viewTimeTableButton')?.addEventListener('click', function() {
    const timeTableUrl = '<?= htmlspecialchars($time_table_url) ?>';
    if (timeTableUrl) {
        window.open(timeTableUrl, '_blank'); // Open the timetable in a new tab
    } else {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if ($.fn.dataTable.isDataTable('#studentTable')) {
        $('#studentTable').DataTable().destroy(); // Destroy existing DataTable if it exists
    }
    ...
    $('#studentTable').DataTable({
        ...
        language: {
            emptyTable: "No students found.", // Custom message for empty table
        },
    });
    ...
    rows.forEach(row => {
        ...
        // Double-click to open the student's profile
        row.addEventListener("dblclick", function() {
            const userId = this.getAttribute("data-user-id");
            if (userId) {
                window.location.href =
                    `<?php echo base_url(); ?>student/student_profile/${userId}`;
            } else {
