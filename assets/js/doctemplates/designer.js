  ...
  return Object.keys(S.layerOff || {})
    ...
    .map(id => {
      ...
      return {
        ...
        /* NO CLIENT TIMESTAMP.
         *
         * Every other timestamp on a stored template — createdAt, updatedAt,
         * renderedAt, publishedAt — is generated by the server. I first wrote
         * an `excludedAt` here from the browser clock, which would have made
         * this the single client-stamped value in the document, on the one
         * record an auditor reads to see WHEN a school set aside a statutory
         * requirement. A laptop with a wrong clock, or a deliberately altered
         * one, would date a legal decision incorrectly.
         *
         * When it happened is already recorded twice, both server-side: the
         * template's own updatedAt, and the audit log row for the save. */
      };
    });
}
